...
<style>
  ...
  .main-sidebar, .main-sidebar .sidebar{ background-color:#222d32 !important; color:#fff !important; }
  .main-sidebar .sidebar-menu>li>a{ color:#b8c7ce !important; font-weight:500; }
  .main-sidebar .sidebar-menu>li>a:hover{ background-color:#1e282c !important; color:#fff !important; }
  .main-sidebar .header{ color:#4b646f !important; border-bottom:1px solid #1a2226; }
  .main-sidebar .user-panel{ display:none !important; }
  body.sidebar-hidden .main-sidebar{ transform:translateX(-260px); transition:transform .25s ease; }
  body.sidebar-hidden .content-wrapper, body.sidebar-hidden .main-footer{ margin-left:0 !important; transition:margin .25s ease; }
  .main-sidebar{ transition:transform .25s ease; }

  /* Permitir que el dropdown de sugerencias salga del contenedor */
  .sidebar .treeview-menu{ overflow: visible; }

  /* ===== Filtros dinámicos (sidebar oscuro AdminLTE) ===== */
  .filtros-dinamicos{ padding:8px 10px; }
  .f-block{ padding:8px 6px; border-bottom:1px solid #1a2226; }
  .f-block:last-child{ border-bottom:0; }

  .f-label{ display:block; font-size:12px; color:#9fb3bf; margin-bottom:6px; }
  .f-input-wrap{ position:relative; }
  .f-input{
    width:100%; background:#1e282c; color:#e6eef2; border:1px solid #2b3940;
    outline:0; padding:8px 10px; border-radius:6px; font-size:13px;
  }
  .f-input:focus{ border-color:#3ea6ff; box-shadow:0 0 0 2px rgba(62,166,255,.2); }

  /* Sugerencias (dropdown) */
  .f-suggest{
    position:absolute; left:0; right:0; top:calc(100% + 4px);
    z-index:9999; list-style:none; margin:0; padding:0;
    background:#1e282c; border:1px solid #2b3940; border-radius:6px;
    max-height:180px; overflow:auto; display:none;
  }
  .f-suggest li{ padding:8px 10px; cursor:pointer; color:#cfe1ea; }
  .f-suggest li:hover{ background:#22313a; }

  /* Categorías */
  .f-cats{ display:grid; gap:6px; }
  .f-cat{ display:flex; align-items:center; gap:6px; color:#cfe1ea; font-size:13px; cursor:pointer; }
  .f-cat input{ margin:0; }

  /* Rango doble */
  .f-range-wrap{ position:relative; height:32px; margin-top:2px; }
  .f-range-track{ position:absolute; left:0; right:0; top:14px; height:6px; border-radius:999px; background:#23313a; }
  .f-range{ position:absolute; inset:0; pointer-events:none; }
  .f-range input{
    pointer-events:auto; -webkit-appearance:none; appearance:none; background:transparent; width:100%; height:32px; margin:0;
  }
  .f-range input::-webkit-slider-thumb{
    -webkit-appearance:none; width:16px; height:16px; border-radius:50%; background:#e6eef2; border:2px solid #111;
  }
  .f-range input::-moz-range-thumb{
    width:16px; height:16px; border-radius:50%; background:#e6eef2; border:2px solid #111;
  }
  .f-row{ display:grid; grid-template-columns:1fr 1fr; gap:6px; margin-top:6px; }
  .f-hint{ font-size:12px; color:#9fb3bf; margin-top:4px; }

  /* Swatches color */
  .f-swatches{ display:grid; grid-template-columns:repeat(6, 1fr); gap:8px; }
  .f-swatch{
    width:22px; height:22px; border-radius:50%; border:2px solid #2b3940; cursor:pointer; outline:0;
  }
  .f-swatch.active{ box-shadow:0 0 0 2px #3ea6ff, 0 0 0 4px #0b1216; }

  /* Radios */
  .f-radio{ display:flex; align-items:center; gap:6px; color:#cfe1ea; font-size:13px; cursor:pointer; }
  .f-radio input{ margin:0; }

  /* Chips */
  .f-chips{ display:flex; flex-wrap:wrap; gap:6px; }
  .f-chip{
    background:#26343c; color:#cfe1ea; border:1px solid #2b3940; border-radius:999px;
    font-size:12px; padding:4px 8px; display:inline-flex; align-items:center; gap:6px;
  }
  .f-chip .x{ cursor:pointer; color:#9fb3bf; }
  .f-chip .x:hover{ color:#fff; }

  /* Botones */
  .f-actions{ padding:10px 6px; display:flex; gap:8px; }
  .f-btn{
    background:#3ea6ff; color:#0b1216; border:0; padding:8px 10px; border-radius:6px; font-weight:600; cursor:pointer; flex:1;
  }
  .f-btn.ghost{ background:transparent; color:#cfe1ea; border:1px solid #2b3940; }
...
document.addEventListener('DOMContentLoaded', () => {
  (function(){
    ...
    suggBox.addEventListener('click', e => {
      ...
      qInput.value = li.dataset.v; state.q = li.dataset.v; suggBox.style.display='none';
      drawChips();
    });
    ...
    async function loadCats(){
      try{
        ...
        // Fallback
        renderCats([
          {id:1, nombre:'Ropa y Accesorios', count:124},
          {id:2, nombre:'Camperas', count:36},
          {id:3, nombre:'Calzados', count:58},
          {id:4, nombre:'Electrónica', count:91}
        ]);
      }
    }
    ...
    // -------- Precio doble rango ----------
    const minR = $('#f-min'), maxR = $('#f-max');
    const minI = $('#f-min-i'), maxI = $('#f-max-i');
    const track = $('#f-price-track'), label = $('#f-price-label');
    const MIN = +minR.min, MAX = +maxR.max;
    ...
    function syncFromRange(){
      ...
      if(a > b-5000){ a=b-5000; minR.value=a; }
      minI.value=a; maxI.value=b; state.min=a; state.max=b; paint(a,b); drawChips();
    }
    function syncFromInput(){
      let a=clamp(+minI.value||MIN,MIN,MAX), b=clamp(+maxI.value||MAX,MIN,MAX);
      if(a > b-5000) a=b-5000;
      minR.value=a; maxR.value=b; state.min=a; state.max=b; paint(a,b); drawChips();
    }
    ...
    $$('#f-colors .f-swatch').forEach(btn => {
      btn.addEventListener('click', ()=>{
        ...
        if(state.colors.has(name)){
          state.colors.delete(name); btn.classList.remove('active');
        }else{
          state.colors.add(name); btn.classList.add('active');
        }
        drawChips();
      });
    });
    ...
    $$('.f-radio-group input[name="estado"]').forEach(r => {
      r.addEventListener('change', ()=>{
        state.estado = r.value; drawChips();
      });
    });
    ...
    function drawChips(){
      chips.innerHTML='';
      if(state.q) makeChip(state.q, ()=>{ state.q=''; qInput.value=''; drawChips(); });

      state.cats.forEach(v => makeChip(v, ()=>{
        state.cats.delete(v);
        catsBox.querySelectorAll('input[type="checkbox"]').forEach(cb => { if(cb.value===v) cb.checked=false; });
        drawChips();
      }));

      state.colors.forEach(v => makeChip('Color: '+v, ()=>{
        state.colors.delete(v);
        $$('#f-colors .f-swatch').forEach(s => { if(s.dataset.name===v) s.classList.remove('active'); });
        drawChips();
      }));

      if(state.estado) makeChip('Estado: '+state.estado, ()=>{
        state.estado=''; $$('.f-radio-group input[name="estado"]').forEach(r=>{ if(r.value==='') r.checked=true; });
        drawChips();
      });
      ...
      if(state.min>MIN || state.max<MAX){
        makeChip(`Gs. ${state.min.toLocaleString('es-PY')} – ${state.max.toLocaleString('es-PY')}`, ()=>{
          ...
          minR.value=MIN; maxR.value=MAX; minI.value=MIN; maxI.value=MAX; paint(MIN,MAX); drawChips();
        });
      }
    }
    drawChips();
    ...
    $('#f-clear').addEventListener('click', ()=>{
      ...
      state.cats.clear(); catsBox.querySelectorAll('input[type="checkbox"]').forEach(cb => cb.checked=false);
      state.colors.clear(); $$('#f-colors .f-swatch').forEach(s => s.classList.remove('active'));
      state.estado=''; $$('.f-radio-group input[name="estado"]').forEach(r => { if(r.value==='') r.checked=true; });
      state.min=MIN; state.max=MAX; minR.value=MIN; maxR.value=MAX; minI.value=MIN; maxI.value=MAX; paint(MIN,MAX);
      drawChips();
      dispatch();
    });
    ...
    function dispatch(){
      ...
      window.dispatchEvent(new CustomEvent('ofertapp:filtros', { detail: payload }));
